...
import { fetchRecords, applyUserActions, getWidgetOptions, setWidgetOption } from '$core/grist.js'
import { normalize, normalizeFields, dateToInput, addMonths, ORGANISMOS, TIPOS_ASAMBLEA, MODALIDAD_CUOTA } from '$core/utils.js'
...
const initComision = async () => {
  ...
  try {
    ...
    if (toCreate.length === 0) {
      bs.setNotice('No hay cargos obligatorios pendientes de inicializar.')
      return
    }
    ...
    await loadComision()
    bs.setNotice(`${toCreate.length} cargo(s) obligatorio(s) inicializado(s).`)
  } catch (e) {
    bs.setError(e?.message || String(e))
  }
}
...
const saveComision = async () => {
  ...
  try {
    if (!tAutoridades) {
      bs.setError('No se encontró la tabla autoridades. Ejecutá "Actualizar schema" en Inicio.')
      return
    }
    const missing = rows.filter((r) => r.cargoObligatorio && r.activo && !r.apellido_nombre.trim())
    if (missing.length > 0) {
      bs.setError(`Faltan cargos obligatorios: ${missing.map((r) => r.cargoNombre).join(', ')}`)
      return
    }
    const actions = []
    for (const r of rows) {
    ...
    await applyUserActions(actions)
    bs.setNotice('Comisión guardada.')
    await loadComision()
  } catch (e) {
    bs.setError(e?.message || String(e))
  } finally {
