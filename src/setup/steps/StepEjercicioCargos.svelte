            ...
            {#each grupo as cargo, i (cargo._uid)}
              <div class="flex flex-wrap items-center gap-2 p-2.5 rounded-lg border border-border bg-muted/5">
                ...
                <div class="flex items-end gap-1">
                  ...
                  <Button variant="ghost" size="icon" aria-label="Bajar" disabled={i === grupo.length - 1} onclick={() => store.reordenar(org, i, 1)}>
                    <ChevronDownIcon />
                  </Button>
                  {#if !cargo.cargo_obligatorio}
                    <Button variant="ghost" size="icon" aria-label="Eliminar" onclick={() => store.removeCargo(cargo._uid)}>
                      <TrashIcon />
                    </Button>
                  {/if}
                </div>
