<script>
  ...
  const load = async () => {
    ...
    if (!isInGrist()) { loading = false; return }
    try {
      await gristReady()
  ...
  const loadCargos = async () => {
    ...
    cargos = all.filter((c) => String(c.organismo) === organismo).sort((a, b) => Number(a.orden || 0) - Number(b.orden || 0))
  }
  ...
  const saveCooperadora = async () => {
    ...
    try {
      ...
      notice = 'Datos guardados.'; notify.success(notice)
    } catch (e) { error = e?.message || String(e); notify.error(error) } finally { busy = false }
  }

  const createEjercicio = async () => {
    notice = ''; error = ''; busy = true
    try {
      if (!tEjercicios) { error = 'No se encontró la tabla ejercicios.'; return }
      ...
      const actions = [...toDeactivate.map((id) => ['UpdateRecord', tEjercicios, id, { en_curso: false }]), ['AddRecord', tEjercicios, null, fields]]
      await applyUserActions(actions)
      ejercicios = await fetchRecords(tEjercicios)
      notice = 'Ejercicio creado.'; notify.success(notice)
      nuevoEj = { anio_inicio: '', anio_fin: '', mes_inicio: nuevoEj.mes_inicio || 'Marzo', saldo_inicial_banco: 0, saldo_inicial_efectivo: 0, saldo_inicial_caja_chica: 0 }
    } catch (e) { error = e?.message || String(e) } finally { busy = false }
  }
  ...
  const setEjercicioEnCurso = async (id) => {
    ...
    try {
      ...
      await applyUserActions(actions)
      ejercicios = await fetchRecords(tEjercicios)
      notice = 'Ejercicio actualizado.'; notify.success(notice)
    } catch (e) { error = e?.message || String(e) } finally { busy = false }
  }
  ...
  const saveCargo = async (c) => {
    notice = ''; error = ''; busy = true
    try {
      if (!tCargos) { error = 'No se encontró la tabla cargos.'; return }
      const fields = normalizeFields({ organismo: c.organismo, nombre_cargo: c.nombre_cargo, nivel: c.nivel, orden: Number(c.orden || 0), duracion_meses: c.duracion_meses === '' ? '' : Number(c.duracion_meses || 0), cargo_obligatorio: Boolean(c.cargo_obligatorio), activo: Boolean(c.activo) })
      if (c.cargo_obligatorio) fields.activo = true
      await applyUserActions([['UpdateRecord', tCargos, c.id, fields]])
      await loadCargos()
      notice = 'Cargo guardado.'; notify.success(notice)
    } catch (e) { error = e?.message || String(e) } finally { busy = false }
  }
  ...
  const addCargo = async () => {
    ...
    try {
      if (!tCargos) { error = 'No se encontró la tabla cargos.'; return }
      if (!String(nuevoCargo.nombre_cargo || '').trim()) { error = 'Completá el nombre del cargo.'; return }
      const fields = normalizeFields({ organismo, nombre_cargo: String(nuevoCargo.nombre_cargo).trim(), nivel: nuevoCargo.nivel, orden: Number(nuevoCargo.orden || 0), duracion_meses: nuevoCargo.duracion_meses === '' ? '' : Number(nuevoCargo.duracion_meses || 0), cargo_obligatorio: Boolean(nuevoCargo.cargo_obligatorio), activo: Boolean(nuevoCargo.activo) })
      if (fields.cargo_obligatorio) fields.activo = true
      await applyUserActions([['AddRecord', tCargos, null, fields]])
      nuevoCargo = { nombre_cargo: '', nivel: nuevoCargo.nivel || 'Titular', orden: Number(nuevoCargo.orden || 10) + 1, duracion_meses: Number(nuevoCargo.duracion_meses || 12), cargo_obligatorio: false, activo: true }
      await loadCargos()
      notice = 'Cargo agregado.'; notify.success(notice)
    } catch (e) { error = e?.message || String(e) } finally { busy = false }
  }

  onMount(() => {
    const unsub = subscribeRecords(() => { if (!busy && !loading) load() })
    load()
    return unsub
  })
</script>
...
{:else}
  <div class="flex flex-col gap-4 max-w-3xl">
    ...
    <Card.Root>
      ...
      <Card.Content class="flex flex-col gap-4">
        <div class="grid gap-3 sm:grid-cols-2">
          <div><Label for="distrito">Distrito</Label><Input id="distrito" bind:value={escuela.distrito} class="mt-1" /></div>
          <div><Label for="escuela-nombre">Escuela</Label><Input id="escuela-nombre" bind:value={escuela.escuela_nombre} class="mt-1" /></div>
          <div><Label for="escuela-numero">Número</Label><Input id="escuela-numero" bind:value={escuela.escuela_numero} class="mt-1" /></div>
          <div><Label for="cue">CUE</Label><Input id="cue" bind:value={escuela.cue} class="mt-1" /></div>
          <div><Label for="cuit">CUIT</Label><Input id="cuit" bind:value={escuela.cuit} class="mt-1" /></div>
          <div><Label for="coop-nombre">Cooperadora</Label><Input id="coop-nombre" bind:value={escuela.cooperadora_nombre} class="mt-1" /></div>
          <div><Label for="coop-dom">Domicilio</Label><Input id="coop-dom" bind:value={escuela.domicilio} class="mt-1" /></div>
          <div><Label for="coop-loc">Localidad</Label><Input id="coop-loc" bind:value={escuela.localidad} class="mt-1" /></div>
          <div><Label for="coop-email">Email</Label><Input id="coop-email" bind:value={escuela.email_cooperadora} class="mt-1" /></div>
          <div><Label for="coop-tel">Teléfono</Label><Input id="coop-tel" bind:value={escuela.telefono_cooperadora} class="mt-1" /></div>
        </div>
        <div class="flex justify-end"><Button onclick={saveCooperadora} disabled={busy}>Guardar</Button></div>
      </Card.Content>
    </Card.Root>
    ...
    <Card.Root>
      ...
      <Card.Content class="flex flex-col gap-4">
        <div class="grid gap-3 sm:grid-cols-3">
          <div><Label for="banco-entidad">Entidad</Label><Input id="banco-entidad" bind:value={banco.entidad} class="mt-1" /></div>
          <div><Label for="banco-cbu">CBU</Label><Input id="banco-cbu" bind:value={banco.cbu} class="mt-1" /></div>
          <div><Label for="banco-cc">Cuenta</Label><Input id="banco-cc" bind:value={banco.cuenta_corriente} class="mt-1" /></div>
        </div>
        <div class="flex justify-end"><Button onclick={saveCooperadora} disabled={busy}>Guardar</Button></div>
      </Card.Content>
    </Card.Root>
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          ...
          <div>
            ...
            <select id="kiosco-modalidad" bind:value={kiosco.modalidad} class="mt-1 h-9 w-full rounded-md border border-input bg-background px-3 text-sm">
              <option value="">(sin)</option><option value="Propio">Propio</option><option value="Licitado">Licitado</option>
            </select>
          </div>
        </div>
        <div class="flex justify-end"><Button onclick={saveCooperadora} disabled={busy}>Guardar</Button></div>
    ...
    <Card.Root>
      ...
      <Card.Content class="flex flex-col gap-4">
        <div><Label for="user-name">Nombre de usuario (para registros)</Label><Input id="user-name" bind:value={userName} placeholder="Ej: Juan Pérez" class="mt-1" /></div>
        <div class="flex justify-end"><Button onclick={saveCooperadora} disabled={busy}>Guardar</Button></div>
      </Card.Content>
    </Card.Root>
    ...
    <Card.Root>
      ...
      <Card.Content class="flex flex-col gap-4">
        ...
        <div class="flex justify-end"><Button size="sm" onclick={createEjercicio} disabled={busy}>Crear y activar</Button></div>
      </Card.Content>
    </Card.Root>
    ...
    <Card.Root>
      ...
      <Card.Content class="flex flex-col gap-4">
                  ...
                  <Table.Row>
                    ...
                    <Table.Cell>
                      <select bind:value={c.nivel} class="h-8 rounded-md border border-input bg-background px-2 text-sm">
                        {#each NIVELES_CARGO as n}<option value={n}>{n}</option>{/each}
                      </select>
                    </Table.Cell>
        ...
        <div class="grid gap-3 sm:grid-cols-3">
          <div><Label for="nc-nombre">Nombre</Label><Input id="nc-nombre" bind:value={nuevoCargo.nombre_cargo} class="mt-1" /></div>
          <div><Label for="nc-duracion">Duración (meses)</Label><Input id="nc-duracion" type="number" bind:value={nuevoCargo.duracion_meses} class="mt-1" /></div>
          <div>
          ...
          <div><Label for="nc-orden">Orden</Label><Input id="nc-orden" type="number" bind:value={nuevoCargo.orden} class="mt-1" /></div>
          <div class="flex flex-col gap-1"><Label class="text-xs font-medium text-muted-foreground">Obligatorio</Label><Checkbox bind:checked={nuevoCargo.cargo_obligatorio} class="mt-1" /></div>
          <div class="flex flex-col gap-1"><Label class="text-xs font-medium text-muted-foreground">Activo</Label><Switch bind:checked={nuevoCargo.activo} disabled={nuevoCargo.cargo_obligatorio} class="mt-1" /></div>
        </div>
        <div class="flex justify-end"><Button size="sm" onclick={addCargo} disabled={busy}>Agregar</Button></div>
      </Card.Content>
    </Card.Root>
  </div>

  {#if error}
    <Alert variant="destructive" class="mt-4"><AlertDescription>{error}</AlertDescription></Alert>
  {/if}
{/if}
